...
import { read, initStore, write } from '@/utils/store.js';
...
function handleSettingChange(updater) {
  ...
  renderMenu();
  // Ensure the style className is applied to the #ui element
  const menuUI = shadowRoot?.querySelector('#ui');
  if (menuUI) {
    menuUI.className = `style-${settings.ui_.menuStyle_}`;
  }
}
...
const scheduleSettingsLoad = () => {
  ...
  setTimeout(() => {
    try {
      ...
      markConfigLoaded();
      settingsLoaded = true;
      // Apply style class to menu element after settings are loaded
      const menuUI = shadowRoot?.querySelector('#ui');
      if (menuUI) {
        menuUI.className = `style-${settings.ui_.menuStyle_}`;
      }
