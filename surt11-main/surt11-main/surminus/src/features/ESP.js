...
const drawFlashlight = (
  ...
  if (!isLocalPlayer) {
    ...
    // Make own flashlight bolder/brighter
    finalOpacity = opacity * 3.5;
  }

  if (isLocalPlayer) {
    const underlayColor = 0xaaaaaa;
    graphics.beginFill(underlayColor, opacity * 3);
...
function renderPlayerLines(localPlayer, players, graphics) {
  ...
  players.forEach((player) => {
    ...
    // Calculate distance
    const distX = player[translations.pos_].x - playerX;
    const distY = player[translations.pos_].y - playerY;
    const distance = Math.sqrt(distX * distX + distY * distY).toFixed(0);
    ...
    const midX = toX * 0.5;
    const midY = toY * 0.5;
    const labelBgColor = isDowned ? 0xffff00 : lineColor;
    
    // Draw knockdown indicator if downed
    if (isDowned) {
      graphics.lineStyle(2.5, 0xffff00, 0.9);
      graphics.drawCircle(toX, toY, 8);
    }
  });
}
