...
function nameTag(player) {
  ...
  Reflect.defineProperty(player.nameText, 'visible', {
    get: () => enabled, // Supprimé le || (isSameTeam && player != localPlayer)
    set: () => { },
  });

  if (!enabled) {
    // Reset au style par défaut quand désactivé
    player.nameText.tint = 0xffffff;
    player.nameText.style.fill = '#ffffff';
    return;
  }
...
const drawFlashlight = (
  ...
  const rayCount = Math.max(10, Math.ceil(weapon.shotSpread)); // Réduit de 30 à 10
...
function renderPlayerLines(localPlayer, players, graphics) {
  ...
  // Filtrer seulement les ennemis (pas les alliés)
  const enemies = players.filter((player) => {
    ...
    if (findTeam(player) === playerTeam) return false; // Ignorer les alliés
    return true;
  });

  // Calculer la distance et trier par proximité
  const enemiesWithDistance = enemies.map((player) => {
    // Optimization: Direct access to coordinates to avoid v2 overhead (though trivial here, good practice)
    const pos = player[translations.pos_];
    const dx = pos.x - playerX;
    const dy = pos.y - playerY;
    return { player, distance: Math.hypot(dx, dy) };
  });

  // Trier par distance et garder les 5 plus proches
  enemiesWithDistance.sort((a, b) => a.distance - b.distance);
  const closestEnemies = enemiesWithDistance.slice(0, 5);

  // Dessiner les lignes
  closestEnemies.forEach(({ player }) => {
    const isOnEffectiveLayer = meetsLayerCriteria(player.layer, localLayer, isLocalOnBypassLayer);
    const isDowned = player.downed;
    const lineColor = isOnEffectiveLayer && !isDowned ? COLORS.RED_ : COLORS.WHITE_;
...
function renderFlashlights(localPlayer, players, graphics) {
  ...
  const enemies = players.filter((player) => {
    ...
    return findTeam(player) !== playerTeam;
  });

  // Trier par distance et garder les 5 plus proches
  const enemiesWithDistance = enemies.map((player) => {
    const dx = player[translations.pos_].x - playerX;
    const dy = player[translations.pos_].y - playerY;
    return { player, distance: Math.hypot(dx, dy) };
  });
  ...
  closestEnemies.forEach(({ player }) => {
    const enemyWeapon = findWeapon(player);
    const enemyBullet = findBullet(enemyWeapon);
    drawFlashlight(localPlayer, player, enemyBullet, enemyWeapon, graphics, 0, 0.05);
  });
}
