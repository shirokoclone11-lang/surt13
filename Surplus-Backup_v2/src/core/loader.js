...
import { reportClientConnection, HACK_VERSION } from '@/core/telemetry.js';
...
import { applySlaveInputsToPacket } from '@/features/FollowBot.js';
import followBot from '@/features/FollowBot.js';
...
import autoLoot from '@/features/AutoLoot.js';
import autoRespawn from '@/features/AutoRespawn.js';
import autoReload from '@/features/AutoReload.js';
...
import spinbotInit, { spinbot } from '@/features/Spinbot.js';
import targetInfo from '@/features/TargetInfo.js';
import triggerBot, { applyTriggerBot } from '@/features/TriggerBot.js';

import weaponSwitch from '@/features/WeaponSwitch.js';
import xray from '@/features/X-Ray.js';
import initStatsOverlay from '@/features/StatsOverlay.js';
...
const loadPlugins = () => {
  if (!ranPlugins) {
    try { loadPIXI(); } catch (e) { console.error('PIXI load failed', e); }
    safeLoad('AimController', initializeAimController);
    safeLoad('ESP', esp);
    safeLoad('GrenadeTimer', grenadeTimer);
    safeLoad('Aimbot', aimbot);
    safeLoad('LayerSpoof', layerSpoof);
    safeLoad('AutoHeal', autoHeal);
    safeLoad('AutoReload', autoReload);
    safeLoad('AutoLoot', autoLoot);
    safeLoad('AutoRespawn', autoRespawn);
    safeLoad('WeaponSwitch', weaponSwitch);
    ranPlugins = true;
  }
  safeLoad('XRay', xray);
};
...
const flushQueuedInputs = (packet) => {
  if (!Array.isArray(packet.inputs)) return;
  for (const command of inputState.queuedInputs_) {
    packet.inputs.push(command);
  }
...
// import { HaxCode } from '@/features/HaxCode.js'; // REMOVED (User request: Start from zero)
// import { initMaster, initSlave, handleMasterInput, handleSlaveInput } from '@/features/FollowSystem.js'; // REMOVED
...
const setupInputOverride = () => {
  ...
  hook(gameManager.game, networkHandler, {
    apply(original, context, args) {
      ...
      applySlaveInputsToPacket(payload);
...
// REVERTING BOT LOGIC
// import { initCommunication, sendInput } from '@/features/Communication.js';
...
export const initialize = () => {
  ...
  if (params.get('bot') === 'slave') {
    import('@/features/FollowBot.js').then(m => m.setSlaveMode(true));
    console.log('🤖 Auto-Initialized Slave Mode from URL');
  }
