  ...
  return new Promise((resolve) => {
    ...
    function matchGameProperties() {
      ...
      function matchSignature(obj, prop) {
        ...
        // Renderer Heuristic: Look for an ARRAY property of length >= 4 (layers)
        // containing PIXI Containers (objects with 'children').
        if (!translated.renderer_) {
          const props = Object.values(val);
          const layersCandidate = props.find(p => Array.isArray(p) && p.length >= 4 && p[0] && Array.isArray(p[0].children));
          if (layersCandidate) {
            console.log("Translator: Found renderer as", prop);
            translated.renderer_ = prop;
            // Don't return, one object might be multiple things? No.
          }
        }

        // SmokeBarn Heuristic: Look for an Array property (particles) inside an object with few properties.
        // And usually has a function update(dt).
        if (!translated.smokeBarn_ && !translated.renderer_) { // Prevent confusion with renderer
          const keys = Object.keys(val);
          if (keys.length > 2 && keys.length < 10) {
            const arrs = Object.values(val).filter(v => Array.isArray(v));
            if (arrs.length === 1) { // Exactly one array (particles)
              // Check if functions exist (addParticle, update)
              const funcs = Object.values(val).filter(v => typeof v === 'function');
              if (funcs.length >= 2) {
                console.log("Translator: Found smokeBarn as", prop);
                translated.smokeBarn_ = prop;
              }
            }
          }
        }

        // Original Signature Matching (as fallback)
      ...
      try {
        if (
          ...
          /*nst objectProps = Object.getOwnPropertyNames(game[translated.map_]).filter(
      ...
      try {
        if (translated.objectCreator_ != null && translated.idToObj_ == null) {
          let f = Object.getOwnPropertyNames(
    ...
    const intervalId = setInterval(() => {
      // Use Object.assign for safer propagation although reference used simple assignment
      Object.assign(translations, matchGameProperties());
